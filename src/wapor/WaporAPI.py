...
class __WaPOR_API_class(object):
    ...
    def _query_catalog(self, level):
        if level is None:
            request_url = r'{0}{1}/cubes?overview=false&paged=false'.format(
                self.path_catalog, self.workspaces[self.version])
        else:
            request_url = r'{0}{1}/cubes?overview=false&paged=false&tags=L{2}'.format(
                self.path_catalog, self.workspaces[self.version], level)
...
#        return cube_info
    ...
    def _query_cubeMeasures(self, cube_code, version=1):
        request_url = r'{0}{1}/cubes/{2}/measures?'
        'overview=false&paged=false'.format(self.path_catalog,
    ...
    def _query_cubeDimensions(self, cube_code, version=1):
        request_url = r'{0}{1}/cubes/{2}/dimensions?'
        'overview=false&paged=false'.format(self.path_catalog,
                                            self.workspaces[self.version],
                                            cube_code)
        resp = requests.get(request_url)
